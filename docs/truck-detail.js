...
// Get truck ID from URL (supports both query params and path-based URLs)
function getTruckIdFromUrl() {
    ...
    // Then try path-based URL (e.g., /truck/ft123 or /foodtrucks/truck/ft123)
    const path = window.location.pathname;
    const segments = path.split('/').filter(s => s);
    
    // Look for segment after 'truck'
    const truckIndex = segments.indexOf('truck');
    if (truckIndex >= 0 && truckIndex < segments.length - 1) {
        return segments[truckIndex + 1];
    }
